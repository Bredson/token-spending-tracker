...
- Każda linia to jeden obiekt JSON reprezentujący jedno zdarzenie w rozmowie. Pola istotne dla parsera (zweryfikowane na realnym pliku logu):
  - `type`: rozróżnia rodzaj zdarzenia. Wartości istotne dla nas: `"user"` (wiadomość użytkownika — początek nowego zadania) i `"assistant"` (odpowiedź modelu — źródło zużycia tokenów). Inne wartości (np. `"queue-operation"`) są pomijane przez parser.
  - `sessionId`: identyfikator sesji (string, zgodny z nazwą pliku).
  - `uuid` / `parentUuid`: każdy wpis ma swój `uuid` i wskazuje `parentUuid` poprzedniego wpisu w łańcuchu rozmowy — to jest podstawa grupowania w `taskId` (patrz niżej).
  - `isSidechain`: `true` dla wpisów pochodzących z wywołania subagenta (np. przez `Agent`/`Task`), `false` dla głównego wątku rozmowy.
...
- **Reguła grupowania w `taskId`**: `taskId` = `uuid` pierwszego wpisu typu `"user"` z `isSidechain: false` w danym łańcuchu; wszystkie kolejne wpisy (w tym `type: "assistant"` i wpisy z `isSidechain: true` — czyli subagenci wywołani w ramach tego zadania) należą do tego samego `taskId`, aż do napotkania kolejnego wpisu `type: "user"`, `isSidechain: false`. Zużycie subagentów jest więc wliczane do zadania nadrzędnego (rozstrzyga to otwarte pytanie z sekcji 8 poprzedniej wersji specyfikacji).
