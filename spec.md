...
- Implementacyjna uwaga o `chokidar` (biblioteka file-watchera): wersja 4.x **usunęła wsparcie dla glob-ów** (`**/*.jsonl` nie działa jako ścieżka do `watch()`) — obserwacja odbywa się na katalogu `~/.claude/projects` rekurencyjnie (domyślne zachowanie v4), a filtrowanie do plików `.jsonl` odbywa się ręcznie w handlerze zdarzenia. `fast-glob` (użyty w `loadAll()`) nie jest tym dotknięty — to osobna biblioteka, glob nadal działa dla jednorazowego wczytania listy plików.
